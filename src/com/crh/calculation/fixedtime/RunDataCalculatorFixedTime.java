...
/**
 * 2014.11.3 “固定时分仿真”  计算运行数据，即rundata
 * @author huhui
 */
public class RunDataCalculatorFixedTime {
     ...
     * @param initV0
     */
    public RunDataCalculatorFixedTime(String routeName, String trainNum, int trainCategoryId, String tractionLevel, double brakeLevel, double maxSpeed,
                                      double runTime, double initV0) {
    ...
    public static void main(String[] args) {
        RunDataCalculatorFixedTime runDataCalculatorMinTime = new RunDataCalculatorFixedTime("京津线（下行）", "G99", 57, "", 9, 300, 1800, 280.46);//275.46
    ...
    public void calculateFullData() {
        ...
        rundataBeansList = new ArrayList<Rundata>(2000); //初始化List用于保存所有正向运行数据
        avgRundataBeansList = new ArrayList<Rundata>(1000);//保存匀速数据
        slowRundataBeansList = new ArrayList<Rundata>(1000);//保存惰行数据
    ...
    public void saveAvgRundataBeans(int runtime, double speed, double location, double tractionPower, double cp, double tractionGridCurrent,
                                    double comBrakeGridCurrent, double acceleration, double tractionForce, double power) {
        Rundata rundata = new Rundata();
        rundata.setRuntime(runtime);
        rundata.setSpeed(speed);
        rundata.setLocation(location);
    ...
    public void mergeForwardRundataList() {
        ...
        double e1 = rundataBeansList.get(rundataBeansList.size() - 1).getPower();
        //合并匀速运行时的数据
        avgRundataBeansList.remove(0);
        for (Rundata bean : avgRundataBeansList) {
            double tPower = bean.getTractionPower() + e0;
            double power = bean.getPower() + e1;
            bean.setTractionPower(tPower);
            bean.setPower(power);
    ...
    public double calAirAndResistancePower(double s1StartLocation, double s1EndLocation, double v, int mode) {
        ...
        double tractionForce = 0, tractionPower = 0, tractionGridCurrent = 0, comBrakeGridCurrent = 0, acceleration = 0, power = 0;
        double location = 0;
        while (true) {
            ...
            System.out.println("currentSpeed=" + currentSpeed + ", tractionForce=" + tractionForce + ", 附加阻力=" + 9.8 * AirFrictionMinTime.m * cp + ", power="
                               + power);*/
    ...
    public double[] calculateT2AndS2(double v0, double v1, double s3EndLocation, int mode) {
        ...
        while (true) {
            ...
            Si = SnMinTime.calShift(currentSpeed, lastSpeed, Si);
            strList.add(str);
            acceleration = CpMinTime.getAcceleretionSlow(currentSpeed);//惰行阶段求加速度
            tractionGridCurrent = netCurrent.getTractionNetCurrent(tractionForce, currentSpeed);
            comBrakeGridCurrent = netCurrent.getComBrakeForceNetCurrent(0, currentSpeed);
            i++;
            lastSpeed = currentSpeed;
