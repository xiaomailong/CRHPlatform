...
/**
 * 2014.10.15 常态运行仿真的“动画运行”
 * @author huhui
 *
 */
public class RealTimeAutoCRHPanel extends JPanel implements Runnable {
	 ...
	 * @param runDataList
	 */
	public RealTimeAutoCRHPanel(int routeId, ArrayList<Rundata> runDataList){
...
//		this.drawRailWay(g);
		...
		drawTable.drawTable(g, MinTimeSimulationPanel.AUTO_MODEL);
	...
	public void run() {
		...
		while (t<runDataList.size()) {
			...
			try {
				...
				this.repaint();
				t = t + 1;
				/*if(runDataList.get(t).getSpeed() == 0){//到站停车
