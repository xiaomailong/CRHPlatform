		...
		calButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(checkParameters()){//参数正常
					...
					RunDataThread runDataThread = new RunDataThread(routeStr, trainNumStr, trainId, maxSpeed, randomSpeed, RunDataThread.FT, initV0, tractionLevel);
//					RunDataThread runDataThread = new RunDataThread("京津线（下行）", "G99", 57, 300.0, 1800, 4, 280); //*调试用
					...
					JOptionPane.showMessageDialog(FixedTimeSimulationPanel.this, "v0 = "+MyTools.numFormat2(bean.getV0())+", v1 = "+MyTools.numFormat2(bean.getV1())+", E = "+MyTools.numFormat2(bean.getE()));
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(MergeLists.ftRundataMergedList != null){
					new RundataChartDialog("固定时分节能控制运行数据曲线", MergeLists.ftRundataMergedList);
				}
			}
		});
					...
					if(MergeLists.ftRundataMergedList.size() != 0){
						try {
							...
							if (retval == JFileChooser.APPROVE_OPTION) {
								...
								CreateDocThread createDocThread = new CreateDocThread(routeStr, trainNumStr, trainId, trainCategory, MergeLists.ftRundataMergedList, savePath);
