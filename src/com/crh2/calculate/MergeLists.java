...
public class MergeLists {
	 ...
	 */
	public MergeLists(ArrayList<ArrayList<Rundata>> forwardList,
			ArrayList<ArrayList<Rundata>> reverseList,
			ArrayList<TrainStopStation> stationList, int routeId) {
	...
	public int[] findIndex() {
		...
		for (int m = 0; m < forwardBeans.size(); m++) {
			...
			for (int n = 0; n < reverseBeans.size(); n++) {
				...
				if ((Math.abs(fBean.getSpeed() - rBean.getSpeed()) <= FACTOR)
						&& (Math.abs(S - fBean.getLocation() - rBean.getDistance()) <= (FACTOR / 60))) {
					flag[0] = m;// 正向
					flag[1] = n;// 反向
	...
	public int[] minTimeFindIndex(int index) {
		...
		int[] flag = new int[2];
		double S = trainStopStationList.get(index + 1).getLocation()
				- trainStopStationList.get(index).getLocation();
		for (int m = 0; m < forwardBeans.size(); m++) {
			Rundata fBean = forwardBeans.get(m);
			for (int n = 0; n < reverseBeans.size(); n++) {
				Rundata rBean = reverseBeans.get(n);
				if ((Math.abs(fBean.getSpeed() - rBean.getSpeed()) <= FACTOR)
						&& (Math.abs(S - fBean.getDistance()
								- rBean.getDistance()) <= (FACTOR / 60))) {
					flag[0] = m;// 正向
					flag[1] = n;// 反向
					// System.out.println(index+"合并成功  p-->"+m+", q-->"+n);
					// System.out.println("fSpeed()="+fBean.getSpeed()+",rSpeed()="+rBean.getSpeed()+", S="+S+", fDistance()="+fBean.getDistance()+", rDistance()="+rBean.getDistance());
	...
	public void displayMergedList(ArrayList<Rundata> rundataList) {
		...
		for (int j = 0; j < rundataList.size(); j++) {
			...
			str = rundata.getRuntime() + "		" + rundata.getSpeed() + "		"
					+ rundata.getDistance() + "		" + rundata.getLocation()
					+ "		" + rundata.getPower();
			strList.add(str);
		}
